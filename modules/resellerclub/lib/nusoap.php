				...
				case 'service':
					switch ($name) {
					    case 'port':
					        ...
					        break;
					    case 'address':
					        //$this->ports[$this->currentPort]['location'] = $attrs['location']; //This line is commented By Manish
					        global $serviceurl;   // This line is added By Manish.
							$this->ports[$this->currentPort]['location'] = $serviceurl;   //This line is added By Manish
							$this->ports[$this->currentPort]['bindingType'] = $namespace;
					        $this->bindings[ $this->ports[$this->currentPort]['binding'] ]['bindingType'] = $namespace;
					        //$this->bindings[ $this->ports[$this->currentPort]['binding'] ]['endpoint'] = $attrs['location']; //This line is commented By Manish
					        $this->bindings[ $this->ports[$this->currentPort]['binding'] ]['endpoint'] = $serviceurl; //This line is added By Manish
					        break;
					} 
					break;
			} 
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false)
	{
		...
		if (strpos($type, ':')) {
			...
			if($ns == $this->XMLSchemaVersion ||
		    	...
		    	if ($uqType == 'boolean' && !$value) {
					$value = 'false'; // Change By Manish To handle boolean $value = 0;
				} elseif ($uqType == 'boolean') {
					$value = 'true'; // Change By Manish To handle boolean $value = 1;
				} 
				...
				if ($uqType == 'Map') {
					...
					foreach($value as $k => $v) {
						...
						$contents .= $this->serialize_val($v,'value',false,false,false,false,$use,$uqType);  //This para is added By Manish $uqType
						$contents .= '</item>';
					}
					if ($use == 'literal') {
		...
		if ($phpType == 'struct') {
			...
			if (isset($this->complexTypes[$uqType]['elements']) && is_array($this->complexTypes[$uqType]['elements'])) {
				...
				// This code is commented By Manish to make Vector element to work
				...
				$p = count($value); // This is added By Manish to read more than one elments of Vector
				// This outer for Loop added By Manish
				for($i=0;$i<$p;$i++)
				{
					...
					if(isset($optionals) && !isset($value[$eName])){
						// do nothing
							...
							// serialize generic type
								...
								$xml .= $this->serialize_val($v, $eName, $this->getLocalPart($attrs['type']), null, $this->getNamespaceFromPrefix($this->getPrefix($attrs['type'])), false, $use,$uqType); //This para is added By Manish $uqType
							}
						}
					} //Add: By Manish
				} 
