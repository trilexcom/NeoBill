	...
	*/
	class DomOrder
	{
		...
		*/
		...
		/**
		* Modifies the Contacts associated with a domain name.
		*
		* @return AssociativeArray
		* @param string Username.
		* @param string Password.
		...
		*/
		...
		function bulkLockCustomerOrders($userName,$password,$role,$langpref,$parentid,$customerIds, $lockName, $add, $reason, $removeChildLocks)
		{
			$para = array($userName,$password,$role,$langpref,$parentid,$customerIds, $lockName, $add, $reason, $removeChildLocks);
			$return = $this->s->call("bulkLockCustomerOrders",$para);
			$this->debugfunction();
			return $return;
		}

		function bulkLockOrders($userName,$password,$role,$langpref,$parentid,$entityIds, $lockName, $add, $reason, $removeChildLocks)
		{
			$para = array($userName,$password,$role,$langpref,$parentid,$entityIds, $lockName, $add, $reason, $removeChildLocks);
			$return = $this->s->call("bulkLockOrders",$para);
			$this->debugfunction();
			return $return;
		}


		function bulkLockResellerOrders($userName,$password,$role,$langpref,$parentid,$resellerIds, $lockName, $add, $reason, $removeChildLocks)
		{
			$para = array($userName,$password,$role,$langpref,$parentid,$resellerIds, $lockName, $add, $reason, $removeChildLocks);
			$return = $this->s->call("bulkLockResellerOrders",$para);
			$this->debugfunction();
			return $return;
		}


		function bulkSuspendCustomerOrders($userName,$password,$role,$langpref,$parentid,$customerIds, $add, $reason, $removeChildLocks)
		{
			$para = array($userName,$password,$role,$langpref,$parentid,$customerIds, $add, $reason, $removeChildLocks);
			$return = $this->s->call("bulkSuspendCustomerOrders",$para);
			$this->debugfunction();
			return $return;
		}

		function bulkSuspendOrders($userName,$password,$role,$langpref,$parentid,$entityIds, $add, $reason, $removeChildLocks)
		{
			$para = array($userName,$password,$role,$langpref,$parentid,$entityIds, $add, $reason, $removeChildLocks);
			$return = $this->s->call("bulkSuspendOrders",$para);
			$this->debugfunction();
			return $return;
		}

		function bulkSuspendResellerOrders($userName,$password,$role,$langpref,$parentid,$resellerIds, $add, $reason, $removeChildLocks)
		{
			$para = array($userName,$password,$role,$langpref,$parentid,$resellerIds, $add, $reason, $removeChildLocks);
			$return = $this->s->call("bulkSuspendResellerOrders",$para);
			$this->debugfunction();
			return $return;
		}


               function checkAvailabilityMultiple($userName,$password,$role,$langpref,$parentid,$DomainNames,$Tlds,$SuggestAlternative)
                {
                        $para = array($userName,$password,$role,$langpref,$parentid,$DomainNames,$Tlds,$SuggestAlternative);
                        $return = $this->s->call("checkAvailabilityMultiple",$para);
                        $this->debugfunction();
                        return $return;
                }
