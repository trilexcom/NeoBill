...
<div class="table">
  {dbo_table dbo_class="AccountDBO" 
             name="inactive_accountdbo_table" 
             filter="status='Inactive'" 
             title="[ACCOUNTS]"
             size="10"}

    {dbo_table_column header="[ID]" sort_field="id"}
      <a target="content" href="manager_content.php?page=accounts_view_account&id={dbo_echo dbo="inactive_accountdbo_table" field="id"}">{dbo_echo dbo="inactive_accountdbo_table" field="id"}</a>
    {/dbo_table_column}

    {dbo_table_column header="[ACCOUNT_NAME]"}
      <a target="content" href="manager_content.php?page=accounts_view_account&id={dbo_echo dbo="inactive_accountdbo_table" field="id"}">{dbo_echo dbo="inactive_accountdbo_table" field="accountname"}</a>
    {/dbo_table_column}

    {dbo_table_column header="[BALANCE]"}
      {dbo_echo|currency dbo="inactive_accountdbo_table" field="balance"}
    {/dbo_table_column}

    {dbo_table_column header="[STATUS]" sort_field="status"}
      {dbo_echo dbo="inactive_accountdbo_table" field="status"}
    {/dbo_table_column}
